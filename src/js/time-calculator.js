...
timeButton.addEventListener("click", () => {
  if (!isNaN(parseInt(timeInput.value))) {
    const secs = Math.floor(Number(Math.abs(timeInput.value)));
    const minutes = Math.floor(Number(Math.abs(timeInput.value / 60)));
    const hours = Math.floor(Number(Math.abs(minutes / 60)));
    const days = Math.floor(Number(Math.abs(hours / 24)));
